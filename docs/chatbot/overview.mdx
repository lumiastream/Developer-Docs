...
image: /img/social/chatbot-commands.png
---
...
3. **User-level gating** - the message sender's badges/role is matched against the command's `userLevels`. Fail means silent skip.
4. **Cooldown check** - per-user and global cooldowns are checked.
5. **Conditions** - any configured conditions on the command's "Conditions" tab are evaluated.
6. **System dispatch** - if the command has a `systemType`, the matching built-in behaviour runs (game logic, mod actions, etc.).
7. **Template resolution** - `chatbot.message` and any side-effect templates are run through the variable resolver.
8. **Side effects** - chatbot reply posted to chat; lights, HFX, sound, etc. fire as configured.
...
Commands can be created, updated, and removed programmatically (e.g. from a Custom JS action or another command's response) using the variable functions:
