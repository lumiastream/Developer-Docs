...
The declaration is one half. Implement `resolveSongRequest` when your service can search its library (skip it for intake-only sources — Lumia then resolves metadata itself and matches your reports by title), plus the transport hooks you support (`playSongRequest`, `enqueueSongRequest`, `removeSongRequest`, `skipSongRequest`, …), and report playback back through the `lumia` bridge: `songRequestNowPlaying`, `songRequestEnded`, and `updateSongRequestQueue`. Plugins can also submit requests into the queue from any playback mode with `addSongRequest`. See the [API Reference](./api-reference).
