...
- **`resolveSongRequest({ query, requesterUsername?, requesterPlatform? }): Promise<{ id, title, artist?, thumbnailUrl?, url?, durationSeconds?, requesterUsername?, requesterPlatform? } | null>`** – optional song-request resolver for source plugins (declared via `config.hasSongRequests` / `config.songRequest`). Map a viewer's query to a track from your service; return `null` when nothing matches so Lumia can reject the request. If you don't implement it (intake-only sources), Lumia accepts requests with its own metadata lookup and matches your `songRequestNowPlaying` reports by requester + title instead of track id. **Throw** when search is temporarily unavailable (service down, no library loaded) so Lumia accepts the request unresolved; return `null` only for a definitive no-match, which rejects it.
