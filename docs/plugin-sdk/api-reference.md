...
- **`resolveSongRequest({ query, requesterUsername?, requesterPlatform? })`** – optional song-request resolver for source plugins (declared via `config.hasSongRequests` / `config.songRequest`). Three outcomes:
  - **Resolved** — return the track `{ id, title, artist?, thumbnailUrl?, url?, durationSeconds?, requesterUsername?, requesterPlatform? }` (or `{ resolved: true, track }`): the request queues with your track identity, and Lumia matches your later reports by `id`.
  - **Accept unresolved** — return `{ resolved: false, track?: { …partial } }`: the request queues anyway. Use this when your source can play things your search can't index (cloud catalogs, no search API). Optional `track` fields are metadata hints that override Lumia's own lookup. Lumia hands the queued track (with a synthetic `id`) to `enqueueSongRequest` — echo that `id` in your `songRequestNowPlaying` reports for an exact match, or Lumia falls back to requester + title.
  - **Reject** — return `null`: the viewer is told the track isn't available.

  If the hook is absent entirely (intake-only sources), every request is accepted unresolved. A **thrown error** is treated as accept-unresolved too (fail-open), so a broken search never eats requests mid-stream.
- **`enqueueSongRequest(track): Promise<void>`** – optional song-request hand-off hook: add a resolved track to your app's queue.
- **`removeSongRequest(track): Promise<void>`** – optional: a single queued request was removed on Lumia's side (mod denied it, the viewer withdrew it, or it was removed from the dashboard). Drop the matching entry from your queue — oldest entry with that track `id`, using `requesterUsername` to disambiguate duplicate requests for the same song.
...
- **`songRequestRemoved({ trackId, requesterUsername? }): Promise<void>`** – report that a single request was removed on your side (denied or withdrawn in your app), so Lumia drops it from its queue too. `requesterUsername` disambiguates duplicate requests for the same track.
...
- **`sendColor(options: { lights?: Array<{ id: string | number; type: string } | string>; color: string | any; power?: boolean; brightness?: number; transition?: number }): Promise<boolean>`** – control connected lighting devices. Omit `lights` to target every connected light. To target specific lights, pass `{ id, type }` entries taken straight from `getLights()` — `type` is the integration key (`hue`, `goveelan`, `wled`, `virtuallights`, …) and is what routes the command, so an id on its own is not enough. Bare-string entries are accepted too and are resolved against `getLights()` by id, alias, or name (`"goveelan:04:74:..."` also works to disambiguate); resolves to `false` without dispatching when none of the requested lights match.
