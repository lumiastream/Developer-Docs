...
export const getAlertSearchText = (alert, config) =>
  [
    ...
    ...(Array.isArray(config?.quickActions) ? config.quickActions : []).map((quickAction) => quickAction?.label),
  ]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
...
export const getVariableExamples = (platform, value) => {
  ...
  if (globalVariableExamples[name]) {
    return globalVariableExamples[name];
  }
...
export const getAlertFieldExamples = (config, field) => {
  ...
  if (field && typeof field === "object" && field.example !== undefined) {
    addExample(field.example);
  }

  (Array.isArray(config?.quickActions) ? config.quickActions : []).forEach((quickAction) => {
    addExample(quickAction?.extraSettings?.[fieldName]);
  });

  (Array.isArray(config?.inputFields) ? config.inputFields : []).forEach((inputField) => {
    if (!inputField || inputField.variableField !== fieldName) {
      return;
    }
          ...
          {variables.map((value) => {
            ...
            return (
              <tr key={name}>
                ...
                <td style={tdStyles}>
                  <code>data.extraSettings.{name}</code>
                </td>
                <td style={tdStyles}>{getDescription(value)}</td>
                <td style={tdStyles}>
                  {examples.length > 0 ? (
              ...
              <>
                <div style={boxStyles}>
                  ...
                  <p style={{ marginTop: 0 }}>
                    ...
                    {" "}
                    <code>{toText(config.message)}</code>
                  </p>
