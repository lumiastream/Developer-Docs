...
| Method | Effect |
| --- | --- |
| `!disablesfx` in chat | Pauses TTS, HFX audio, and AudioManager. Mod/broadcaster only. |
| `!enablesfx` in chat | Resumes the above. |
| `{{set_sfx_muted=true}}` | Same effect from any template, action, or custom JS. |
| `{{set_sfx_muted=false}}` | Re-enable from a template. |
