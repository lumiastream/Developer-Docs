---
sidebar_position: 0
id: alerts-tiktok-index
title: TikTok alerts
description: Every alert Lumia emits for TikTok.
---

# TikTok alerts

20 alerts available. Each page documents the message template, payload fields, and example scenarios.

| Alert | Description |
| --- | --- |
| [Battle End](./battle-end.mdx) | ended a battle |
| [Battle Progress](./battle-progress.mdx) | updated battle progress |
| [Battle Start](./battle-start.mdx) | started a battle |
| [Entrance](./entrance.mdx) | Entrance |
| [First Chatter](./first-chatter.mdx) | was the first chatter |
| [Follower](./follower.mdx) | became a follower |
| [Gift](./gift.mdx) | sent x`{{giftAmount}}` `{{giftName}}` |
| [Like](./like.mdx) | liked the stream `{{userLikeCount}}` times |
| [New Video](./new-video.mdx) | new video |
| [Pin Message](./pin-message.mdx) | pinned a message |
| [Poll](./poll.mdx) | sent a poll message |
| [Question](./question.mdx) | asked a question |
| [Share](./share.mdx) | shared your stream |
| [Shop Purchase](./shop-purchase.mdx) | triggered live shopping |
| [Stream End](./stream-end.mdx) | stream ended |
| [Stream Live](./stream-live.mdx) | stream is now live |
| [Super Fan](./super-fan.mdx) | became a Super Fan |
| [Super Fan Box](./super-fan-box.mdx) | triggered a Super Fan box |
| [Total Likes](./total-likes.mdx) | reached `{{totalLikeCount}}` likes |
| [Treasure Chest](./treasure-chest.mdx) | sent a treasure chest |
